...
class Compose(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs):
        """Returns images that are augmented with the given augmentations."""
        assert img.shape[:-1]==mask.shape
        assert img.shape[:-1]==dmaps.shape
        
        for a in self.augmentations:
            img, mask,dmaps, inputs = a(img, mask,dmaps, *inputs)
        return (img, mask,dmaps, *inputs)
...
class RandomCrop(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        """Returns randomly cropped image."""
        
        if self.padding > 0:
            img = ImageOps.expand(img, border=self.padding, fill=0)
            mask = ImageOps.expand(mask, border=self.padding, fill=0)
            dmaps=ImageOps.expand(dmaps, border=self.padding, fill=0)
        ...
        tra=trans.ToPILImage()
        tens=trans.ToTensor()
        img=np.array(Image.fromarray(img).crop((x1, y1, x1 + tw, y1 + th)))
        mask=np.array(Image.fromarray(mask).crop((x1, y1, x1 + tw, y1 + th)))
        dmaps=np.array(Image.fromarray(dmaps).crop((x1, y1, x1 + tw, y1 + th)))
        return (img,mask,dmaps,
                tuple(np.array(i.crop((x1, y1, x1 + tw, y1 + th))) for i in inputs))
...
class CenterCrop(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        return (img.crop((x1, y1, x1 + tw, y1 + th)),
                ...
                dmaps.crop((x1, y1, x1 + tw, y1 + th))*inputs)
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        if random.random() < self.prob:
            ...
            np.array(Image.fromarray(mask).transpose(Image.FLIP_LEFT_RIGHT)),
            np.array(Image.fromarray(dmaps).transpose(Image.FLIP_LEFT_RIGHT)),
            tuple(i.transpose(Image.FLIP_LEFT_RIGHT) for i in inputs))

        return (img,mask,dmaps,tuple(i for i in inputs))
...
class FreeScale(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        return (img.resize(self.size, Image.BILINEAR),
                ...
                dmaps.resize(self.size, Image.NEAREST), *inputs)
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        if w > h:
            ...
            return (img.resize((ow, oh), Image.BILINEAR),
                    ...
                    dmaps.resize((ow, oh), Image.NEAREST), *inputs)
        else:
            oh = self.size
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        for attempt in range(10):
            ...
            if w <= img.size[0] and h <= img.size[1]:
                ...
                dmaps = dmaps.crop((x1, y1, x1 + w, y1 + h))
                assert (img.size == (w, h))

                return (img.resize((self.size, self.size), Image.BILINEAR),
                        mask.resize((self.size, self.size), Image.NEAREST),
                        dmaps.resize((self.size, self.size), Image.NEAREST), *inputs)

        # Fallback
        scale = Scale(self.size)
        crop = CenterCrop(self.size)
        return crop(*scale(img, mask,dmaps, *inputs))
...
class RandomRotate(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        return (img.rotate(rotate_degree, Image.BILINEAR),
                ...
                dmaps.rotate(rotate_degree, Image.NEAREST), *inputs)
...
class RandomSized(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        img, mask,dmaps = img.resize((w, h), Image.BILINEAR), mask.resize((w, h), Image.NEAREST,),dmaps.resize((w, h), Image.NEAREST,)

        return self.crop(*self.scale(img, mask,dmaps, *inputs))
...
class RandomOcclusion(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        ...
        while (dmaps == self.occlusion_class).sum() < self.min_size:
            self.queue = []
            self.flags = torch.full_like(dmaps, 0).byte()
            self.occlusion_map = torch.full_like(dmaps, 0).byte()
            self.img_height = img.shape[-2]
            self.img_width = img.shape[-1]
            ...
            if self.occlusion_map.sum().item() >= self.min_size:
                for c in range(img.shape[0]):
                    img[c][self.occlusion_map] = 0
                dmaps[self.occlusion_map] = self.occlusion_class
        return (img, mask,dmaps, *inputs)
...
class RandomNoise(object):
    ...
    def __call__(self, image, mask,dmaps, *inputs, **kwargs):
        if random.random() < self.prob:
            image = (1 - self.ratio) * image + self.ratio * torch.rand_like(image)
        return (image, mask,dmaps, *inputs)
...
class RandomNoiseImage(object):
    ...
    def __call__(self, image, mask,dmaps, *inputs, **kwargs):
        if random.random() < self.prob:
            ...
            dmaps = torch.full_like(dmaps, self.class_index)
        return (image, mask,dmaps, *inputs)
...
class ToTensor(object):
    def __call__(self, image, mask,dmaps, *inputs, **kwargs):
        ...
        torch.from_numpy(dmaps),
        tuple(torch.tensor(np.array(i, dtype=np.float32), dtype=torch.long) for i in inputs))
...
class Normalize(object):
    ...
    def __call__(self, tensor, mask,dmaps, *inputs, **kwargs):
        return self.t(tensor), mask,dmaps, tuple(i for i in inputs)
...
class DeStandardize(object):
    ...
    def __call__(self, tensor, mask,dmaps, *inputs, **kwargs):
        ...
        return (tensor, mask,dmaps, *inputs)
...
class Lambda(object):
    ...
    def __call__(self, img, mask,dmaps, *inputs, **kwargs):
        return self.lambd(img, mask,dmaps, *inputs)
