...
class LostAndFound(Dataset):
    ...
    def __init__(self, root: str, split: str = "train", transform: Optional[Callable] = None, shuffle=True)-> None:
        ...
        self.labelTrainIds_test=[] ##list of all labelTrainIDs for ood test
        ...
        for root, _, filenames in os.walk(os.path.join(root, 'leftImg8bit',self.split)):
            for filename in filenames:
                if os.path.splitext(filename)[1] == '.png':
                    ...
                    labbeltest=os.path.join(self.root,'gtCoarse',self.split,city,filename_base +'_gtCoarse_labelTrainIds.png')#original labelTrainIds for test
                    self.labelTrainIds_test.append(labbeltest)

                    #get disparity maps from directory
                    disparity_root=os.path.join(self.root, 'disparity', self.split)
                    disparity_map=os.path.join(disparity_root, city, filename_base + '_disparity.png')
                    self.disparity.append(os.path.join(disparity_root, city, filename_base + '_disparity.png'))
        ...
        self.disparity=natsorted(self.disparity)
    ...
    def __getitem__(self, i):
        ...
        if self.split=='test':
            ...
            image = np.asarray(Image.open(self.images[i])).astype(np.uint8)#.convert('RGB')     
            labelTrainId=np.asarray(Image.open(self.labelTrainIds_test[i])).astype(np.uint8)#.convert('L') 
            ...
            disparity_map=self.disparity[i]
