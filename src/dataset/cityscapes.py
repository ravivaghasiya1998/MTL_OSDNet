...
class Cityscapes(Dataset):
    ...
    num_eval_classes = num_train_ids
    id2label = {label.id: label for label in labels}
    train_id2label = {label.train_id: label for label in labels}
    train_id_in = 1
    train_id_out = 2

    def __init__(self, root: str = "/home/ravi/ravivaghasiya/dataset/cityscapes", split: str = "train", mode: str = "gtFine",
                 target_type: str = "semantic_train_id",shuffle=True, transform: Optional[Callable] = None,
        ...
        self.labelTrainIds_dir = os.path.join(self.root, self.mode, self.split)
        self.disparity_dir=os.path.join(self.root,'disparity',self.split)
        ...
        for city in os.listdir(self.images_dir):
            ...
            labelTrainIds_city_dir = os.path.join(self.labelTrainIds_dir, city)
            disparity_city_dir=os.path.join(self.disparity_dir,city)
            pred_dir = os.path.join(self.predictions_dir, city)
            for file_name in os.listdir(img_dir):
                labelTrainIds_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0],
                                             self._get_target_suffix(self.mode, target_type))
                self.images.append(os.path.join(img_dir, file_name))
                self.labelTrainIds.append(os.path.join(labelTrainIds_city_dir, labelTrainIds_name))
                disparity_name=file_name.split('_leftImg8bit')[0]
                self.disparity.append(os.path.join(disparity_city_dir, disparity_name+'_disparity.png'))
                self.predictions.append(os.path.join(pred_dir, file_name.replace("_leftImg8bit", "")))
        """
        shuffle data and subsample
        """
        if shuffle:
            self.images,self.labelTrainIds,self.disparity=shuffled(self.images,self.labelTrainIds,self.disparity)      

    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        if self.split in ['train']:
            image=self.images[index]
            labelTrainId = self.labelTrainIds[index]
        
        elif self.split == 'val':
            image = np.asarray(Image.open(self.images[index])).astype(np.uint8)   
            labelTrainId=np.asarray(Image.open(self.labelTrainIds[index])).astype(np.uint8)
            disparity_map=cv2.imread(self.disparity[index],cv2.IMREAD_UNCHANGED).astype(np.float32)
