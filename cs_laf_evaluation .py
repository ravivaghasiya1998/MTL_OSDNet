...
from src.cs_laf_model_utils import inference
...
class eval_pixels(object):
    ...
    def __init__(self, params, roots, dataset,config):
        self.params = params
        self.config = config
        ...
        self.result_dir=os.path.join(self.roots.io_root)
        self.save_dir_data = os.path.join(self.roots.io_root, "entropy_counts_per_pixel")
        self.save_dir_plot = os.path.join(self.roots.io_root, "plots")
        self.save_dir_metrics=os.path.join(self.roots.io_root,"metrics")
        if self.epoch == 0:
    ...
    def counts(self, loader, num_bins=100, save_path=None, save_metrics_path=None,rewrite=False,Cityscapes=True):
        ...
        save_disparity=os.path.join(self.result_dir,'Epoch_'+ str(self.epoch),'disparity')
        ...
        if not os.path.exists(save_disparity):
            os.makedirs(save_disparity)
        ...
        if not os.path.exists(save_metrics_path) or rewrite:
            ...
            inf = inference(self.params,self.config,self.roots, loader,  self.dataset.num_eval_classes)
           
            eval_metrics=StreamSegMetrics(n_classes=19)
            eval_metrics.reset()
            eval_rmse=RMSE(ignore_val=0)
            eval_aepe = AEPE(ignore_val=0)
            eval_rmse.reset()
            eval_aepe.reset()
            
            loader=tqdm(loader,leave=True,position=0)
            for i in range(len(loader)):
                probs_seg,probs_disp, gt_train,gt_disp,_,_= inf.probs_gt_load(i)
                eval_metrics.update(gt_train,probs_seg,train=False)
                eval_rmse.update(pred=probs_disp,gt=gt_disp,train=False)
                eval_aepe.update(pred=probs_disp,gt=gt_disp,train=False)
                roi_path='/home/ravi/ravivaghasiya/cs_laf_predict/ood_seg_disp/roi_image.png'
                roi=np.array(Image.open(roi_path).convert('L')).astype(np.float32)

                # pixel-wise classification
                max_index=np.argmax(probs_seg,axis=0)
                max_index=np.asarray(max_index).astype(np.uint8)

                # Generate semantic segmentation (RGB)
                ...
                ##dispparity 
                probs_disp=np.squeeze(probs_disp)
                predict_dis=probs_disp*255
                predict_dis=predict_dis * roi

                 #save disparity prediction
                #plt.imsave(os.path.join(save_disparity,image_name + '_disparity.png'),predict_dis)
                # Calculate entropy 
                ...
                #save entropy maps
                #plt.imsave(os.path.join(save_ent_plt,image_name + '_entropy' + '.png'),ent)

                # save semantic segmentation prediction 
                #plt.imsave(os.path.join(save_segmentation,image_name + '_segmentation' + '.png'),seg)
                ...
                loader.set_description('Evaluation : Epoch %i :Images: %i'%(self.epoch,i+1))
                loader.refresh()               
                sys.stdout.flush()
            ...
            test_RMSE=eval_rmse.val()
            test_AEPE = eval_aepe.val()
            performance_metrics={'mIoU' : miou, 'RMSE' : test_RMSE, 'AEPE':test_AEPE,'Class_IoU':class_iou}
    ...
    def oodd_metrics_pixel(self, datloader=None, load_path=None,load_metrics_path=None):
        ...
        _, _, _, auprc = calc_precision_recall(data, balance=True)
        if self.epoch == 0:
        ...
        print("AEPE:",test_metrics['AEPE'])
        print("Class_IoU:",test_metrics['Class_IoU'])
        print('m_ent_in',test_metrics['m_ent_in'])
        print('m_ent_out',test_metrics['m_ent_out'])
...
def main(args):
    ...
    datloader = config.dataset(root=config.roots.eval_dataset_root,split=config.split, transform=transform)
    ...
    if args["pixel_eval"]:
        ...
        eval_pixels(config.params, config.roots, config.dataset,config=config).oodd_metrics_pixel(datloader=datloader)
