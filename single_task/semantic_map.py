...
def blend_ori_ent(original_img,entropy_map):
    entropy_map= entropy_map*255
    original_img=cv2.cvtColor(original_img,cv2.COLOR_RGB2GRAY)
    original_img=np.array(original_img).astype(np.float32)
    blended_img=cv2.addWeighted(original_img,0.1,entropy_map,0.9,0)
    return blended_img
def extract_road(seg_img):
    mask=np.zeros(seg_img.shape).astype(np.float64)
    roi_path='/home/ravi/ravivaghasiya/cs_laf_predict/ood_seg_disp/roi_image.png'
    roi=np.array(Image.open(roi_path).convert('L')).astype(np.float64)
